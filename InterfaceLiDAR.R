                ...
                navbarPage(
                  ...
                  tabPanel("Segmentation", 
                           sidebarLayout(
                             ...
                             mainPanel(
                               plotOutput("preprocessed_chm_plot"),
                               plotOutput("selected_local_maxima_plot"),
                               plotOutput("segmented_trees_plot")
                             )
                           )
                  )
                ))
...
server <- function(input, output, session) {
  ...
  observeEvent(input$file_filtrage, {
    ...
    output$plotCHM_filtrage <- renderPlot({
      if (input$algorithm_filtrage == "lidR::pitfree" && (is.null(input$param1_filtrage) || is.null(input$param2_filtrage))) {
        ...
        chm <- lidaRtRee::rasterize_canopy(las, 
  ...
  output$preprocessed_chm <- renderPlot({
    req(input$file_segmentation)
    las <- lidR::readLAS(input$file_segmentation$datapath)
    chm <- lidR::rasterize_canopy(las, res = 0.5)
    segm <- lidaRtRee::tree_segmentation(chm)
    plot(segm$smoothed_dem, main = "Pre-processed CHM")
  })
  
  output$selected_local_maxima <- renderPlot({
    req(input$file_segmentation)
    las <- lidR::readLAS(input$file_segmentation$datapath)
    chm <- lidR::rasterize_canopy(las, res = 0.5)
    segm <- lidaRtRee::tree_segmentation(chm)
    plot(segm$local_maxima, main = "Selected Local Maxima")
  })
  
  output$segmented_trees <- renderPlot({
    req(input$file_segmentation)
    las <- lidR::readLAS(input$file_segmentation$datapath)
    chm <- lidR::rasterize_canopy(las, res = 0.5)
    segm <- lidaRtRee::tree_segmentation(chm)
    dummy <- segm$segments_id
    dummy[dummy == 0] <- NA
    plot(dummy, main = "Segmented Trees", col = rainbow(8), type = "classes", legend = FALSE)
  })
  
  # Load and process LAS file for the new tab
  observeEvent(input$lasfile, {
    # Load LAS file
    las <- readLAS(input$lasfile$datapath)
    
    # Compute CHM
    dtm <- rasterize_terrain(las, res = 0.5, tin())
    dsm <- rasterize_canopy(las, res = 0.5, p2r())
    chm <- dsm - dtm
    ...
    output$preprocessed_chm_plot <- renderPlot({
      plot(segms$smoothed_dem, main = "Pre-processed CHM")
    })
    
    # Display selected local maxima
    output$selected_local_maxima_plot <- renderPlot({
      plot(segms$local_maxima, main = "Selected Local Maxima")
    })
    
    # Display segmented trees
    output$segmented_trees_plot <- renderPlot({
      # Exclude ground segment
      dummy <- segms$segments_id
      dummy[dummy == 0] <- NA
      plot(dummy, main = "Segmented Trees (Random Colors)",
